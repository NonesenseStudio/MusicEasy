    ...
    if (isPlainObject(fromArgs)) {
      ...
      for (const key in fromArgs) {
        if (hasOwn(argsOption, key)) {
          ...
          if (!keyOption) {
            console.warn(`鸿蒙元服务 ${methodName} 暂不支持 ${key}`);
...
let deviceId;
function useDeviceId(global2 = has) {
...
function initEventChannel(events, cache = true) {
  id++;
  const eventChannel = new has.EventChannel(id, events);
  if (cache) {
...
function initUni(api, protocols2, platform = has) {
...
function initGetProvider(providers2) {
  return function getProvider2({ service, success, fail, complete }) {
    let res;
    if (providers2[service]) {
      res = {
        errMsg: "getProvider:ok",
        service,
        provider: providers2[service]
      };
...
function callHook(name, args) {
  ...
  {
    if (name === "onLoad" && args && args.__id__ && isFunction(has.getEventChannel)) {
...
function isPage(mpInstance) {
  return !!mpInstance.route || !!(mpInstance._methods || mpInstance.methods || mpInstance).onLoad;
}
...
function handleLink({ detail: { nodeId, webviewId } }) {
  ...
  const mountedVm = function() {
    if (vm._$childVues) {
      vm._$childVues.forEach(([createdVm2]) => createdVm2());
      vm._$childVues.forEach(([, mountedVm2]) => mountedVm2());
      delete vm._$childVues;
    }
    vm.$callHook("mounted");
...
has.createSubpackageApp = global.createSubpackageApp = createSubpackageApp;
